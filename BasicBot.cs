...
namespace Microsoft.BotBuilderSamples
{
    ...
    public class BasicBot : IBot
    {
        ...
        public const string WimbledonIntent = "Wimbledon";
        public const string TwickenhamIntent = "Twickenham";
                    ...
                    switch (dialogResult.Status)
                    {
                        case DialogTurnStatus.Empty:
                            switch (topIntent)
                            {
                                ...
                                case WimbledonIntent:
                                    await DispatchToQnAMakerAsync(turnContext, WimbledonQnAMakerKey);
                                    break;

                                case TwickenhamIntent:
                                    await DispatchToQnAMakerAsync(turnContext, TwickenhamQnAMakerKey);
                                    break;
        ...
        private async Task DispatchToQnAMakerAsync(ITurnContext context, string appName, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (!string.IsNullOrEmpty(context.Activity.Text))
            {
                ...
                {
                    previousIntentAnswer = await _services.QnAServices[previousIntent].GetAnswersAsync(context);
                }
